...
def predict_log_proba_single(
    ...
    if keep_grad:
        X_base = torch.from_numpy(X_base).to(X_query.device)
    ...
    if keep_grad:
        ...
        for k in range(len(classes_cur)):
            log_ps_cur[k] += torch.log(torch.sum(torch.exp(log_kernel_vals[indices[k]])))
        log_ps_total_cur = torch.log(torch.sum(torch.exp(
            torch.cat([
                log_ps_cur,
                log_pN + torch.from_numpy(log_prior[classes_cur]).to(log_ps_cur.device).unsqueeze(1),
            ],
                dim=1)
        ), dim=1))
        # Compute denominator (it is the same for all classes)
        log_denominator = torch.log(torch.sum(torch.exp(log_ps_cur)) + np.exp(log_pN))
        # Select class with top probability
        idx_max = torch.argmax(log_ps_total_cur)
    else:
        log_ps_cur = np.array([logsumexp(log_kernel_vals[indices[k]]) for k in range(len(classes_cur))])
        # Compute numerator for each class
        log_ps_total_cur = logsumexp(np.c_[log_ps_cur, log_pN + log_prior[classes_cur],], axis=1, )
        # Compute denominator (it is the same for all classes)
        log_denominator = logsumexp(np.r_[log_ps_cur, log_pN,])
        # Select class with top probability
    ...
    if log_ps_total_cur[idx_max] > log_prior_default:
        if keep_grad:
            ...
            log_numerator_1mp = torch.tensor(log_pN + np.log1p(-log_prior[idx_max])).to(X_query.device)
            if len(temp) > 0:
...
def predict_log_proba_batch(
    ...
    if keep_grad:
        preds = torch.zeros(size=(size,)).to(X_query.device)
        log_probas = torch.zeros(size=(size,)).to(X_query.device)
        log_uncs = torch.zeros(size=(size,)).to(X_query.device)
    ...
    for j, (X, idx) in enumerate(zip(X_query[sl], idx_query[sl])):
        ...
        if keep_grad:
            preds_cur, log_probas_cur, log_uncs_cur = predict_log_proba(
                X_base[idx], y_base[idx], X, log_kernel
            )
            preds[j] += preds_cur
            log_probas[j] += log_probas_cur
            log_uncs[j] += log_uncs_cur
        else:
...
def get_log_kernel(
        name: str = "RBF", bandwidth: Union[float, np.ndarray] = 1.0, keep_grad=False
) -> Callable:
    ...
    try:
        return getattr(kernels, name.lower())(bandwidth, keep_grad)
