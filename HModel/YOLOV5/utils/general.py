...
from HModel.YOLOV5.utils import TryExcept, emojis
from HModel.YOLOV5.utils.downloads import curl_download, gsutil_getsize
from HModel.YOLOV5.utils.metrics import box_iou, fitness
...
def non_max_suppression(
    ...
    return_ori=False,  # new parameter
    preds_f=None
):
    """
    ...
    Returns:
         If return_ori=False: list of detections, on (n,6) tensor per image [xyxy, conf, cls].
         If return_ori=True: tuple of (output, extracted), where:
             - output: list of detections, on (n,6) tensor per image [xyxy, conf, cls].
             - extracted: list of extracted predictions corresponding to NMS indices.
    """
    ...
    output_s = [torch.zeros((0, 5 + nc), device=prediction.device)] * bs
    extracted = [None] * bs  # Store extracted predictions for return_ori=True
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        if multi_label:
            ...
            # conf_index = conf.view(-1) > conf_thres
            if return_ori:
        ...
        if not n:  # no boxes
            continue
        sort_index = x[:, 4].argsort(descending=True)[:max_nms]
        x = x[sort_index]  # sort by confidence and remove excess boxes
        ...
        if return_ori:
            # 获取原始预测和特征
            extracted[xi] = preds_f[xi][xc[xi]][sort_index][i]  # extract original predictions
            output_s[xi] = x_s[sort_index][i]
        if mps:
    ...
    return (output, output_s, extracted) if return_ori else output
