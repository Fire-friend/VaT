...
class UnalignedDataset(data.Dataset):
    def __init__(self, opt):
        ...
        if opt.phase == 'train':
            self.dir_A = os.path.join(opt.dataroot, opt.phase + 'A_Dark')
            self.dir_B = os.path.join(opt.dataroot, opt.phase + 'B_dark')
        else:
    ...
    def __getitem__(self, index):
        ...
        # show_tensor(A_img.permute([1,2,0])*0.5+0.5)
        # print()
        ...
        A_img_cut, mask_A = self.Cutout(A_img)
        B_img_cut, mask_B = self.Cutout(B_img)
        return {'A': A_img, 'B': B_img, 'input_img': input_img,
                'A_paths': A_path, 'B_paths': B_path, 'A_cutout_mask': mask_A, 'B_cutout_mask': mask_B, }
